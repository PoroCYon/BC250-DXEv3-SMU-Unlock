...
REPO_DIR="$SCRIPT_DIR"
PKG_DIR="$REPO_DIR/BC250DXEv3SMUUnlockPkg"
PLATFORM_DSC="$PKG_DIR/BC250DXEv3SMUUnlockPkg.dsc"
OUTPUT_DIR="$REPO_DIR/Build/Output"
BUILD_PACKAGE_DIR="BC250DXEv3SMUUnlock"
MODULE_INFS=(
  "$PKG_DIR/BC250DXEv3SMUUnlockDxe/BC250DXEv3SMUUnlockDxe.inf"
)
MODULE_NAMES=(
  "MeiMeiDXEv3SMUUnlock"
)
MODULE_GUIDS=(
  "A3F8B2D5-7C4E-4F0A-B1D2-9E6F3C8A5B71"
)
...
build_with_container() {
  ...
  "$CONTAINER_ENGINE" run --rm \
    -v "$REPO_DIR:/workspace/BC250-DXEv3-SMU-Unlock:Z" \
    -v "$CACHE_DIR:$CONTAINER_CACHE_ROOT:Z" \
    -w /workspace/BC250-DXEv3-SMU-Unlock \
    -e ARCH="$ARCH" \
    -e TARGET="$TARGET" \
    ...
    bash -lc '
      ...
      # Only initialize the submodules required to build BaseTools and MdePkg for
      # this driver. A full recursive update can fail on unrelated optional nested
      # dependencies in upstream edk2 (for example OpenSSL test/development trees).
      ...
      export PACKAGES_PATH=/workspace/BC250-DXEv3-SMU-Unlock:"$CONTAINER_EDK2_DIR"
      export PYTHON_COMMAND="${PYTHON_COMMAND:-python3}"
      ...
      build \
        ...
        -b "$TARGET" \
        -t "$TOOL_CHAIN_TAG" \
        -p /workspace/BC250-DXEv3-SMU-Unlock/BC250DXEv3SMUUnlockPkg/BC250DXEv3SMUUnlockPkg.dsc \
        -Y PCD
      NAMES=(MeiMeiDXEv3SMUUnlock)
      GUIDS=(A3F8B2D5-7C4E-4F0A-B1D2-9E6F3C8A5B71)
      for INDEX in 0; do
        ...
        mkdir -p /workspace/BC250-DXEv3-SMU-Unlock/Build/Output
        cp "$EFI_PATH" "/workspace/BC250-DXEv3-SMU-Unlock/Build/Output/${MODULE_NAME}.efi"
        "$CONTAINER_EDK2_DIR/BaseTools/BinWrappers/PosixLike/GenSec" -s EFI_SECTION_PE32 -o "/workspace/BC250-DXEv3-SMU-Unlock/Build/Output/${MODULE_NAME}.pe32.sec" "/workspace/BC250-DXEv3-SMU-Unlock/Build/Output/${MODULE_NAME}.efi"
        "$CONTAINER_EDK2_DIR/BaseTools/BinWrappers/PosixLike/GenSec" -s EFI_SECTION_USER_INTERFACE -n "$MODULE_NAME" -o "/workspace/BC250-DXEv3-SMU-Unlock/Build/Output/${MODULE_NAME}.ui.sec"
        "$CONTAINER_EDK2_DIR/BaseTools/BinWrappers/PosixLike/GenFfs" -t EFI_FV_FILETYPE_DRIVER -g "${GUIDS[$INDEX]}" -o "/workspace/BC250-DXEv3-SMU-Unlock/Build/Output/${MODULE_NAME}.ffs" -i "/workspace/BC250-DXEv3-SMU-Unlock/Build/Output/${MODULE_NAME}.pe32.sec" -i "/workspace/BC250-DXEv3-SMU-Unlock/Build/Output/${MODULE_NAME}.ui.sec"
